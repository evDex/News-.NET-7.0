...
function CloseContextMenu(menu) {
    menu.style.left = '0px';
    menu.style.top = '0px';
    document.getElementById("modDialogXL").removeChild(menu);
...
function CreateContextMenuText(event, menuOptionArray) {
    ...
    document.getElementById("modDialogXL").appendChild(menuElement);
    menuElement.style.display = 'block';
    menuElement.style.left = event.clientX + document.getElementById("modDialogXL").scrollLeft + "px";
    menuElement.style.top = event.clientY + document.getElementById("modDialogXL").scrollTop + "px";
...
function InitDetailBody() {
    var div = document.getElementById('text');
    var inputsText = document.getElementsByClassName('input-article__text');
    var inputsImgPath = document.getElementsByClassName('input-img__path');
    ...
    for (var i = 0; i < inputsText.length; i++) {
        ...
        textArea.className = "textarea-article__text";
        textArea.value = inputsText[i].value;
        textArea.cols = "50";
        textArea.rows = "5";
        textArea.oninput = function () {
            textArea.style.height = (textArea.scrollHeight) + "px";
        };
        divText.addEventListener('contextmenu', function (event) {
            if (currentMenuVisible === null) {
                event.preventDefault();
                CreateContextMenuText(event, ['copy', 'paste', 'trash']);
            }
        });
        $(divText).data("div-type", "text");
        divText.appendChild(textArea);
        div.appendChild(divText);

        if (i < inputsImgPath.length) {
            CreateImgDiv(true, inputsImgPath[i].value);
        }
    }
}
function CreateImgDiv(state, isCreate) {
    var div = document.getElementById('text');
